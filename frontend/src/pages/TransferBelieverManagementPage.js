...
const TransferBelieverManagementPage = () => {
  const { user } = useAuth();
  // 상태 관리
  const [transferBelievers, setTransferBelievers] = useState([]);
  const [openDialog, setOpenDialog] = useState(false);
  const [editingTransferBeliever, setEditingTransferBeliever] = useState(null);
  const [formData, setFormData] = useState({
    name: '',
    phone: '',
    department: '새가족위원회', // 고정값
    believer_type: '전입신자', // 고정값
    education_type: '',
    year: '',
    gender: '',
    birth_date: '',
  ...
  const [filteredTransferBelievers, setFilteredTransferBelievers] = useState([]);
  ...
  // 전입신자 목록 가져오기 (조회 조건 포함)
  const fetchTransferBelievers = async (searchParams = null) => {
    try {
      let url = '/api/transfer-believers';
      ...
      if (searchParams) {
        ...
        // 전입신자관리는 항상 전입신자만 조회하므로 believer_type 조건은 제거
      ...
      const transferBelieversData = Array.isArray(data) ? data : [];
      setTransferBelievers(transferBelieversData);
      setFilteredTransferBelievers(transferBelieversData); // 조회 결과를 필터링된 데이터로 설정
    } catch (error) {
      console.error('전입신자 목록 가져오기 실패:', error);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = editingTransferBeliever 
        ? `/api/transfer-believers/${editingTransferBeliever.id}`
        : '/api/transfer-believers';
      
      const method = editingTransferBeliever ? 'PUT' : 'POST';
      
      // 교육 상태가 수료에서 다른 상태로 변경된 경우 전송확인 상태 초기화
      let submitData = { ...formData };
      if (editingTransferBeliever && editingTransferBeliever.education_type === '수료' && formData.education_type !== '수료') {
      ...
      if (response.ok) {
        ...
        setOpenDialog(false);
        resetForm();
        fetchTransferBelievers(searchConditions);
      } else {
        const errorData = await response.json();
        alert(errorData.error || '오류가 발생했습니다.');
      }
    } catch (error) {
      console.error('전입신자 저장 실패:', error);
      alert('전입신자 저장 중 오류가 발생했습니다.');
    }
  };

  // 삭제 처리
  const handleDelete = async (transferBelieverId) => {
    // 삭제할 전입신자 정보 조회
    const transferBeliever = transferBelievers.find(tb => tb.id === transferBelieverId);
    
    // 수료자인 경우 삭제 불가
    if (transferBeliever && transferBeliever.education_type === '수료') {
    ...
    if (transferBeliever && transferBeliever.file_id) {
      confirmMessage = '파일이 연결된 전입신자입니다. 파일도 함께 삭제하시겠습니까?';
    }
    
    if (window.confirm(confirmMessage)) {
      try {
        const response = await fetch(`/api/transfer-believers/${transferBelieverId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        ...
        alert('전입신자 삭제 중 오류가 발생했습니다.');
      }
    }
  };

  // 수정 모드로 변경
  const handleEdit = (transferBeliever) => {
    setEditingTransferBeliever(transferBeliever);
    ...
    // 전송 버튼 표시 여부 설정
    const shouldShowTransferButton = transferBeliever.education_type === '수료' && 
      (transferBeliever.graduate_transfer_status === '' || transferBeliever.graduate_transfer_status === null || transferBeliever.graduate_transfer_status === '미전송');
    setShowTransferButton(shouldShowTransferButton);
    
    // 기존 번호 저장
    setOriginalNumber(transferBeliever.number || '');
    
    setFormData({
      department: '새가족위원회', // 고정값
      believer_type: transferBeliever.believer_type || '전입신자', // 수정 시 실제 값 사용
      education_type: transferBeliever.education_type || '',
      year: transferBeliever.year || '',
      name: transferBeliever.name || '',
      gender: transferBeliever.gender || '',
      marital_status: transferBeliever.marital_status || '',
      birth_date: formatDateForInput(transferBeliever.birth_date),
      address: transferBeliever.address || '',
      phone: transferBeliever.phone || '',
      teacher: transferBeliever.teacher || '',
      register_date: formatDateForInput(transferBeliever.register_date),
      education_start_date: formatDateForInput(transferBeliever.education_start_date),
              education_end_date: formatDateForInput(transferBeliever.education_end_date),
      affiliation_org: transferBeliever.affiliation_org || '',
      belong: transferBeliever.belong || '',
      new_life_strategy_date: formatDateForInput(transferBeliever.new_life_strategy_date),
      identity_verified: transferBeliever.identity_verified || '',
      prev_church: transferBeliever.prev_church || '',
      comment: transferBeliever.comment || '',
      number: transferBeliever.number || '',
      graduate_transfer_status: transferBeliever.graduate_transfer_status || '',
      file_id: transferBeliever.file_id || null
    });
    setSelectedFile(null);
      setUploadedFile(null);
    
    // 기존 파일 정보 로드
    if (transferBeliever.file_id) {
      fetchFileInfo(transferBeliever.file_id);
    }
    
    // 교사 목록 로드 (수정 시에는 실제 값 사용)
    fetchTeachers('새가족위원회', transferBeliever.believer_type || '전입신자');
  ...
  // 새 전입신자 추가
  const handleAdd = () => {
    setEditingTransferBeliever(null);
    resetForm();
    setOpenDialog(true);
  };
  ...
  const resetForm = () => {
    setFormData({
      department: '새가족위원회', // 고정값
      believer_type: '전입신자', // 전입신자관리는 전입신자로 고정
  ...
  const handleGraduateTransfer = async () => {
    ...
    try {
      ...
      // 신자구분은 전입신자로 고정
      transferData.believer_type = '전입신자';
      ...
      console.log('전입신자 ID:', editingTransferBeliever.id);

      // 중복이 없는 경우 기존 전송 로직 실행
      const response = await fetch(`/api/transfer-believers/${editingTransferBeliever.id}/graduate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleSearch = async () => {
    try {
      ...
      await fetchTransferBelievers(searchConditions);
    } catch (error) {
  ...
  const handleExcelDownload = () => {
    ...
    const excelData = transferBelievers.map((item, index) => {
      const convertedItem = {
        'No': index + 1 // No 컬럼 추가
      };
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '전입신자관리');
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    const fileName = `전입신자관리_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const handleExcelUpload = async (event) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // 한글 헤더를 영문 필드명으로 변환하는 매핑
        const headerMapping = {
          '이름': 'name',
          '부서': 'department',
          '신자': 'believer_type',
          '교육': 'education_type',
          '전송확인': 'graduate_transfer_status',
          '년도': 'year',
          '번호': 'number',
          '성별': 'gender',
          '결혼': 'marital_status',
        ...
        const convertedData = jsonData.map(row => {
          ...
          // 기본값 설정
          converted.department = '새가족위원회';
          converted.believer_type = '전입신자';
        ...
        const response = await fetch('/api/transfer-believers/upload', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  useEffect(() => {
    fetchCodeData();
    fetchTransferBelievers(); // 초기 로드시 전체 데이터 가져오기
    // 초기 교사 목록 로드 (고정값 사용)
    fetchTeachers('새가족위원회', '전입신자');
    // 공통파일 이미지 가져오기
  ...
  return (
    <Box sx={{ p: 3, mt: 6 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 2, mt: -7.5, alignItems: 'center' }}>
        <Tooltip title="새 전입신자 추가" arrow placement="top">
          <IconButton
            onClick={handleAdd}
            size="small"
            sx={{
      ...
      <div className="ag-theme-alpine" style={{ 
        ...
        <AgGridReact
          ...
          rowData={(() => {
            if (Array.isArray(filteredTransferBelievers) && filteredTransferBelievers.length > 0) {
              return filteredTransferBelievers;
            }
            if (Array.isArray(transferBelievers)) {
              return transferBelievers;
            }
            return [];
          })()}
      ...
      >
        총 {(() => {
          if (Array.isArray(filteredTransferBelievers) && filteredTransferBelievers.length > 0) {
            return filteredTransferBelievers.length;
          }
          if (Array.isArray(transferBelievers)) {
            return transferBelievers.length;
          }
          return 0;
        })()}건
      </Box>
      ...
      >
        ...
        >
          {editingTransferBeliever ? '전입신자 수정' : '새 전입신자 추가'}
        </DialogTitle>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mt: 1 }}>
            ...
            <TextField
              ...
              onChange={async (e) => {
                ...
                // 전입신자에서 초신자로 변경하는 경우 번호 업데이트
                if (oldBelieverType === '전입신자' && newBelieverType === '초신자') {
                  try {
                    // 새로운 번호 생성 요청
                    ...
                    if (response.ok) {
                      ...
                      console.log('초신자 번호 자동 생성:', data.number);
                    } else {
                ...
                // 초신자에서 전입신자로 변경하는 경우 기존 번호 복원
                else if (oldBelieverType === '초신자' && newBelieverType === '전입신자') {
                  console.log('초신자 → 전입신자: 기존 번호 복원:', originalNumber);
                  setFormData({
                    ...formData, 
                    believer_type: newBelieverType, 
                    teacher: '',
            ...
            <TextField
              ...
              size="small"
              required
              disabled={editingTransferBeliever && formData.education_type === '수료'}
              sx={{
                '& .MuiOutlinedInput-root': {
                  borderRadius: '12px',
                  background: 'rgba(255, 255, 255, 0.8)',
        ...
        <DialogActions sx={{ padding: '20px 24px', gap: 2 }}>
          ...
          {editingTransferBeliever && showTransferButton && (
            <Button 
      ...
      >
        ...
        >
          프린트 미리보기 - {selectedRows.length}명의 전입신자
        </DialogTitle>
        <DialogContent sx={{ padding: '20px' }}>
          ...
          <Button
            onClick={async () => {
              ...
              const printContent = `
                ...
                <head>
                  <meta charset="UTF-8">
                  <title>프린트 - ${selectedRows.length}명의 전입신자</title>
                  <style>
                    @page {
                      size: A4 portrait;
                      margin: 0;
                  ...
                  ${selectedRows.map((row, index) => `
                    <div class="print-page">
                      <div class="photo-section">
                        ${row.file_id && selectedFilesInfo[row.id || row.file_id] ? 
                          `<img src="http://localhost:3001/uploads/${getRelativePath(selectedFilesInfo[row.id || row.file_id].saved_path)}" alt="${row.name || '전입신자'} 사진" />` : 
                          row.file_id ? 
        ...
        <DialogActions sx={{ padding: '20px 24px', gap: 2 }}>
          ...
          <Button 
            onClick={async () => {
              ...
              if (success) {
                // 전입신자 상태도 수료로 변경하고 전송확인을 '전송'으로 설정
                try {
                  const response = await fetch(`/api/transfer-believers/${editingTransferBeliever.id}`, {
                    method: 'PUT',
                    headers: {
                      'Content-Type': 'application/json',
                      'Authorization': `Bearer ${localStorage.getItem('token')}`
                  ...
                  if (!response.ok) {
                    console.error('전입신자 상태 업데이트 실패');
                  }
              } catch (error) {
                  console.error('전입신자 상태 업데이트 실패:', error);
                }
              }
            }}
            variant="contained"
...
export default TransferBelieverManagementPage;

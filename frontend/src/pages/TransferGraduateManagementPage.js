...
const TransferGraduateManagementPage = () => {
  const { user } = useAuth();
  // 상태 관리
  ...
  const [searchConditions, setSearchConditions] = useState({
    year: new Date().getFullYear().toString(), // 현재 년도를 기본값으로 설정
    name: '',
    believer_type: '전입신자', // 기본값을 전입신자로 설정
    phone: ''
  });
  ...
  const columnDefs = [
    ...
    { headerName: '년도', field: 'year', width: 100, minWidth: 100, maxWidth: 120, sortable: true, filter: true },
    { headerName: '부서', field: 'department', width: 150, minWidth: 120, maxWidth: 2000, sortable: true, filter: true },
    { headerName: '신자', field: 'believer_type', width: 110, minWidth: 110, maxWidth: 130, sortable: true, filter: true },
    ...
    { headerName: '이름', field: 'name', width: 80, minWidth: 80, maxWidth: 120, sortable: true, filter: true },
    ...
    { 
      ...
      sortable: true, 
      filter: true,
      valueFormatter: (params) => params.value === '1' ? '확인완료' : '미확인'
    },
  ...
  // 수료자 목록 가져오기 (전입신자 수료자만 필터링)
  const fetchGraduates = async (searchParams = {}) => {
    try {
      const queryParams = new URLSearchParams();
      // 기본적으로 전입신자 수료자만 조회
      queryParams.append('believer_type', '전입신자');
      ...
      const url = `/api/transfer-graduates?${queryParams.toString()}`;
  ...
  const handleExcelDownload = () => {
    ...
    // 수료번호 데이터 확인 로그
    console.log('엑셀 다운로드 - 수료번호 데이터 확인:');
    graduates.forEach((item, index) => {
      console.log(`행 ${index + 1}: graduate_number = ${item.graduate_number}`);
    });
    ...
    const excelData = graduates.map((item, index) => {
      ...
      Object.keys(headerMapping).forEach(key => {
        if (key === 'no') {
          row[headerMapping[key]] = index + 1;
        } else {
          const value = item[key];
          if (key === 'identity_verified') {
            row[headerMapping[key]] = value === '1' ? '확인완료' : '미확인';
          } else if (key === 'graduate_number') {
            // 수료번호 필드 특별 처리
            row[headerMapping[key]] = value || '';
            console.log(`수료번호 처리: ${key} = ${value} -> ${row[headerMapping[key]]}`);
          } else if (key.includes('date') && value) {
            row[headerMapping[key]] = new Date(value).toLocaleDateString('ko-KR');
          } else {
            row[headerMapping[key]] = value || '';
          }
        }
    ...
    XLSX.utils.book_append_sheet(wb, ws, '전입신자수료자');
    XLSX.writeFile(wb, `전입신자수료자_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (jsonData.length > 0) {
          ...
          console.log('수료번호 컬럼 확인:', {
            '수료번호': firstRow['수료번호'],
            'graduate_number': firstRow['graduate_number'],
        ...
        const convertedData = jsonData.map(row => {
          ...
          Object.keys(converted).forEach(key => {
            ...
            if (mappedKey) {
              mappedData[mappedKey] = converted[key];
            } else {
              mappedData[key] = converted[key];
            }
          });
          ...
          ['birth_date', 'register_date', 'education_start_date', 'education_end_date', 'new_life_strategy_date'].forEach(field => {
            if (mappedData[field] !== undefined && mappedData[field] !== null) {
              mappedData[field] = convertDateField(mappedData[field]);
            }
          });
          
          // 전입신자로 신자구분 강제 설정
          mappedData['believer_type'] = '전입신자';
          
          // 수료번호 필드 확인 로그
          console.log('수료번호 필드 처리:', {
            original: converted['수료번호'],
            mapped: mappedData['graduate_number']
          });
        ...
        const response = await fetch('/api/transfer-graduates/upload', {
          method: 'POST',
          headers: {
  ...
  const handleResetSearch = () => {
    setSearchConditions({
      year: new Date().getFullYear().toString(), // 현재 년도를 기본값으로 설정
      name: '',
      believer_type: '전입신자', // 전입신자는 유지
      phone: ''
    });
    fetchGraduates({ believer_type: '전입신자', year: new Date().getFullYear().toString() });
  };
...
export default TransferGraduateManagementPage;
